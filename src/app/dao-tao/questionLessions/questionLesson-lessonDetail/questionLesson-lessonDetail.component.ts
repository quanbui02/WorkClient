...
@Component({
  selector: 'app-questionLesson-lessonDetail',
  templateUrl: './questionLesson-lessonDetail.component.html',
  styleUrls: ['./questionLesson-lessonDetail.component.scss']
})
export class QuestionLessonLessonDetailComponent extends SecondPageEditBase
  implements OnInit {
  isLoading = false;
  modelEdit: any = {};
  page = 1;
  ...
  async showPopup(id: any) {
    ...
    await this._questionLessonsService.Gets(
      "", id, (this.page - 1) * this.limit, this.limit, "", false, true).then(rs => {
        if (rs.status) {
  ...
  //   this._questionLessonsService.Gets(
  //     "", this.idLesson, (this.page) * this.limit, this.limit, "", false, true).then(rs => {
  //       if (rs.status) {
  //         this.dataSource = rs.data;
  //         this.total = rs.totalRecord;
  //       }
