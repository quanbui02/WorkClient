...
export class ThemeComponent extends SecondPageIndexBase implements OnInit {
    searchModel: any = {
        key: '',
        isActive: -1,
        type: -1
    ...
    @ViewChild(ThemeEditComponent) _ThemeEditComponent: ThemeEditComponent;
    ...
    async getData() {
        this.isLoading = true;
        this.dataSource = [];
        await this._ThemesService.Gets(
    ...
    onActive(item: any, e) {
        ...
        this._ThemesService.Active(obj).then(rs => {
            this._notifierService.showSuccess(rs.message);
        });
    }
