...
export class NewsCategoryComponent extends SecondPageIndexBase implements OnInit {
  searchModel: any = {
    key: ''
  };
  dataTree: any = [];
  ...
  async onLoadCategories() {
    await this._NewsCategoriesService.searchDropdown('').then(rs => {
  ...
  async getData() {
    this.isLoading = true;
    this.dataSource = [];
    await this._NewsCategoriesService.Gets(
  ...
  async nodeSelect(event) {
    if (this.selectedTreeNode.data > 0) {
      await this._NewsCategoriesService.getDetail(this.selectedTreeNode.data)
        .then(async response => {
          if (response.status) {
  ...
  save() {
    this._NewsCategoriesService.post(this.modelEdit).then(rs => {
      if (rs.status) {
        var obj = {
          label: rs.data.name,
          data: rs.data.id,
  ...
  onDelete(item: any) {
    this._notifierService.showConfirm('Xóa bản ghi?', 'Bạn có chắc muốn xóa bản ghi này?').then(rs => {
      this._NewsCategoriesService.delete(item.data).then(re => {
        if (re.status) {
