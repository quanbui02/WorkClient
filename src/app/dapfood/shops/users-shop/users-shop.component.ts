...
@Component({
    ...
    styleUrls: ['./users-shop.component.scss']
})
export class UsersShopComponent extends SecondPageEditBase implements OnInit {
    active_options: any[];
    modelEdit: any = {};
    cols: any[];
    isLoading: any;
    ...
    async getListRole() {
        var lstRoleUser: any = [];
        this.listRolesGroup = [];
        await this._vaiTroService.search('SHOP', 1, 0, 9999).then(async rs => {
            if (rs.status) {
                lstRoleUser = rs.data;
            }
        });
    ...
    async getData() {
        await this.userService.getUsersByIdShop(this.modelEdit.idClient).then(async rs => {
            if (rs.status) {
    ...
    async addNew() {
        ...
        this.modelEdit.RegType = 3;
        this.modelEdit.IdType = 3;
