...
@Component({
    selector: 'app-multi-file-upload-fbstyle',
    templateUrl: './multi-file-upload-fbstyle.component.html',
    styleUrls: ['./multi-file-upload-fbstyle.component.scss'],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => VsMultiFileUploadFbStyleComponent),
            multi: true
        }
    ],
    encapsulation: ViewEncapsulation.None
})
export class VsMultiFileUploadFbStyleComponent extends SecondPageEditBase implements OnInit, ControlValueAccessor {
    id: number;
    disabled = false;
    apiUrl = '';
    maxFileSize = 1000000;
            ...
            this._fileUploadService.GetsByIds(obj).then(rs => {
                if (rs.status) {
                    for (let i = 0; i < rs.data.length; i++) {
                        ...
                        fileItem.linkViewOnline = this.urlImageOriginal(fileItem.path, fileItem.fileName); // this._fileUploadService.getLinkDownload(fileItem.id);
